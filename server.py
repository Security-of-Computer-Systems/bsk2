...
def serve_edit_film():
    ...
    # find name of the user making the request and check if user has write permission to film with id <id>
    cur.execute("""
            SELECT username 
            FROM Users 
...
def serve_add_film():
    ...
    # find name of the user making the request
    cur.execute("""
    SELECT username 
    FROM Users 
    ...
    # Add film and permissions to database.
    if username is not None:
...
def serve_login():
    ...
    # find password hash in database of the user making the request
    ...
    if result is not None and result[0] == password:
        ...
        # create sessionID for user and put in database
...
def get_films_with_permissions():
    ...
    # find all user's films and permissions for them
...
def get_films():
    ...
    # find all user's films to which he has delegation or ownership permission
...
def get_film(id):
    ...
    # check if user has read permission to film with id <id>
...
def can_delegate(edge, delegation, read, write):
    ...
    if delegation:
        ...
        for cycle in nx.simple_cycles(tmp_graph):
            return False
        # check if node has more than 1 predecessors
...
def set_permissions():
    ...
    # find donor's username
    cur.execute("""SELECT username FROM Users  
        WHERE sessionID = %s
        """, (sessionID,))
    donor_username = cur.fetchone()[0]

    # find donor permissions to film with id <id>
    ...
    #  Check if user has delegation permission and   permissions he wants to transfer
    if result is not None and result[0] == True and (read <= result[1]) and (write <= result[2]):
        ...
        if can_delegate((donor_username, username2), delegation, read, write):
            ...
            # Check if there is relation between user and film
...
def transfer_ownership():
    ...
    if result is not None and result[0] == True and (delegation <= result[1]) and (read <= result[2]) and (write <= result[3]):
        ...
        # removing all permissions of donor
        cur.execute("UPDATE Permissions SET read = %s, write = %s, delegation = %s, ownership = %s WHERE FK_Film = %s and FK_User = %s",
